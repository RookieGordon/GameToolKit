...
namespace BehaviorDesigner.Runtime
{
  ...
  public class BehaviorManager : MonoBehaviour
  {
      ...
      Transform behaviorTransform)
    {
      ...
      data.Initialize();
      bool hasExternalBehavior = (UnityEngine.Object) behavior.ExternalBehavior != (UnityEngine.Object) null;
    ...
    {
      if ((UnityEngine.Object) behavior == (UnityEngine.Object) null || behaviorTree == null)
        return;
      if (!string.IsNullOrEmpty(behaviorTree.errorState))
      {
        UnityEngine.Debug.LogError((object) behaviorTree.errorState);
      }
      else
      {
        this.dirty = true;
        if ((UnityEngine.Object) behavior.ExternalBehavior != (UnityEngine.Object) null)
        ...
        {
          for (int index = 0; index < behaviorTree.taskList.Count; ++index)
            UnityEngine.Debug.Log((object) string.Format("{0}: Task {1} ({2}, index {3}) {4}", (object) this.RoundedTime(), (object) behaviorTree.taskList[index].FriendlyName, (object) behaviorTree.taskList[index].GetType(), (object) index, (object) behaviorTree.taskList[index].GetHashCode()));
        }
        for (int index = 0; index < behaviorTree.taskList.Count; ++index)
      ...
      BehaviorManager.TaskAddData data)
    {
      ...
      if (task is BehaviorReference)
      {
        ...
        for (int index1 = 0; index1 < behaviorSourceArray.Length; ++index1)
        {
          ...
          lock ((object) this.lockObject)
            behaviorSourceArray[index1].CheckForSerialization(true, behaviorSource, true);
                ...
                if (behaviorReference.variables[index2] != null)
                {
                  ...
                  if (!data.overrideFields.ContainsKey(behaviorReference.variables[index2].Value.name))
                  {
                    ...
                    if (behaviorReference.variables[index2].Value != null)
                    {
                      ...
                      {
                        UnityEngine.Debug.LogWarning((object) ("Warning: Named variable on reference task " + behaviorReference.FriendlyName + " (id " + (object) behaviorReference.ID + ") is null"));
                        continue;
                      }
                    ...
                    else if (behaviorReference.variables[index2].Value != null)
                    {
                      ...
                      if (genericVariable.value != null)
                      {
                        ...
                        if (data.overrideFields.TryGetValue(genericVariable.value.Name, out overrideFieldValue3))
      ...
      else
      {
        ...
        if (task is ParentTask)
        {
          ...
          for (int index5 = 0; index5 < count; ++index5)
          {
            ...
            behaviorTree.parentCompositeIndex.Add(data.compositeParentIndex);
            int taskList;
            if ((taskList = this.AddToTaskList(behaviorTree, parentTask.Children[index5], behaviorGameObject, behaviorTransform, ref hasExternalBehavior, data)) < 0)
            {
              if (taskList == -3)
              {
      ...
      for (int index1 = 0; index1 < serializableFields.Length; ++index1)
      {
        ...
        if (obj1 != null)
        {
          ...
          else if (typeof (IList).IsAssignableFrom(serializableFields[index1].FieldType))
          {
            ...
            if ((typeof (SharedVariable).IsAssignableFrom(fieldType = serializableFields[index1].FieldType.GetElementType()) || serializableFields[index1].FieldType.IsGenericType && typeof (SharedVariable).IsAssignableFrom(fieldType = serializableFields[index1].FieldType.GetGenericArguments()[0])) && obj1 is IList list)
            {
              for (int index2 = 0; index2 < list.Count; ++index2)
              {
                SharedVariable sharedVariable = this.OverrideSharedVariable(behaviorTree, data, fieldType, list[index2] as SharedVariable);
                if (sharedVariable != null)
                  list[index2] = (object) sharedVariable;
              }
            }
          }
          else if (serializableFields[index1].FieldType.IsClass && !serializableFields[index1].FieldType.Equals(typeof (System.Type)) && !typeof (Delegate).IsAssignableFrom(serializableFields[index1].FieldType) && !data.overiddenFields.Contains(obj1))
          {
            data.overiddenFields.Add(obj1);
            this.OverrideFields(behaviorTree, data, obj1);
            data.overiddenFields.Remove(obj1);
          }
          if (TaskUtility.HasAttribute(serializableFields[index1], typeof (InspectTaskAttribute)))
          {
            if (typeof (IList).IsAssignableFrom(serializableFields[index1].FieldType))
            {
              if (typeof (Task).IsAssignableFrom(serializableFields[index1].FieldType.GetElementType()) && obj1 is IList list1)
              {
                for (int index3 = 0; index3 < list1.Count; ++index3)
                  this.OverrideFields(behaviorTree, data, (object) (list1[index3] as Task));
              }
            }
    ...
    public void DisableBehavior(Behavior behavior, bool paused, TaskStatus executionStatus)
    {
      if (!this.IsBehaviorEnabled(behavior))
      {
        ...
        else
        {
          ...
          {
            if ((UnityEngine.Object) this.activeThreads[index].Behavior == (UnityEngine.Object) behavior)
            {
      ...
      if (behavior.LogTaskChanges)
        UnityEngine.Debug.Log((object) string.Format("{0}: {1} {2}", (object) this.RoundedTime(), !paused ? (object) "Disabling" : (object) "Pausing", (object) ((object) behavior).ToString()));
      if (paused)
      {
        BehaviorManager.BehaviorTree behaviorTree;
    ...
    public bool IsBehaviorEnabled(Behavior behavior) => this.behaviorTreeMap != null && this.behaviorTreeMap.Count > 0 && (UnityEngine.Object) behavior != (UnityEngine.Object) null && behavior.ExecutionStatus == TaskStatus.Running;
    ...
    {
      if ((UnityEngine.Object) behavior == (UnityEngine.Object) null || !this.IsBehaviorEnabled(behavior))
        return;
      ...
      for (int index1 = 0; index1 < behaviorTree.conditionalReevaluate.Count; ++index1)
      {
        if (behaviorTree.conditionalReevaluate[index1].compositeIndex != -1)
        {
          ...
          if (taskStatus != behaviorTree.conditionalReevaluate[index1].taskStatus)
          {
            ...
            for (int index5 = behaviorTree.conditionalReevaluate.Count - 1; index5 > index1 - 1; --index5)
            {
              BehaviorManager.BehaviorTree.ConditionalReevaluate conditionalReevaluate = behaviorTree.conditionalReevaluate[index5];
              if (this.FindLCA(behaviorTree, compositeIndex, conditionalReevaluate.index) == compositeIndex)
              {
                behaviorTree.taskList[behaviorTree.conditionalReevaluate[index5].index].NodeData.IsReevaluating = false;
            ...
            {
              BehaviorManager.BehaviorTree.ConditionalReevaluate conditionalReevaluate = behaviorTree.conditionalReevaluate[index6];
              ...
              {
                behaviorTree.taskList[behaviorTree.conditionalReevaluate[index6].index].NodeData.IsReevaluating = false;
              ...
              {
                for (int index7 = 0; index7 < behaviorTree.childrenIndex[compositeIndex].Count; ++index7)
                {
                  if (this.IsParentTask(behaviorTree, behaviorTree.childrenIndex[compositeIndex][index7], conditionalReevaluate.index))
                  {
                    int index8 = behaviorTree.childrenIndex[compositeIndex][index7];
                    while (!(behaviorTree.taskList[index8] is Composite) && behaviorTree.childrenIndex[index8] != null)
            ...
            for (int index9 = behaviorTree.parentIndex[index2]; index9 != compositeIndex; index9 = behaviorTree.parentIndex[index9])
            ...
            for (int index10 = this.conditionalParentIndexes.Count - 1; index10 > -1; --index10)
            {
              ParentTask task2 = behaviorTree.taskList[this.conditionalParentIndexes[index10]] as ParentTask;
              if (index10 == 0)
                task2.OnConditionalAbort(behaviorTree.relativeChildIndex[index2]);
              else
                task2.OnConditionalAbort(behaviorTree.relativeChildIndex[this.conditionalParentIndexes[index10 - 1]]);
            }
      ...
      TaskStatus previousStatus)
    {
      ...
      {
        if (behaviorTree.behavior.LogTaskChanges)
          MonoBehaviour.print((object) string.Format("{0}: {1}: Skip task {2} ({3}, index {4}) at stack index {5} (task disabled)", (object) this.RoundedTime(), (object) ((object) behaviorTree.behavior).ToString(), (object) behaviorTree.taskList[taskIndex].FriendlyName, (object) behaviorTree.taskList[taskIndex].GetType(), (object) taskIndex, (object) stackIndex));
      ...
      if ((UnityEngine.Object) this.breakpointTree != (UnityEngine.Object) null)
      ...
      TaskStatus status)
    {
      ...
      if (!task.CanRunParallelChildren() || task.OverrideStatus(TaskStatus.Running) != TaskStatus.Running)
      {
        ...
        while (task.CanExecute() && (taskStatus != TaskStatus.Running || task.CanRunParallelChildren()) && this.IsBehaviorEnabled(behavior) && behaviorTree.childrenIndex.Count > taskIndex)
        {
          ...
          else
          {
            if (this.executionsPerTick == BehaviorManager.ExecutionsPerTickType.NoDuplicates && num3 == num2 || this.executionsPerTick == BehaviorManager.ExecutionsPerTickType.Count && behaviorTree.executionCount >= this.maxTaskExecutionsPerTick)
            {
              if (this.executionsPerTick == BehaviorManager.ExecutionsPerTickType.Count)
                UnityEngine.Debug.LogWarning((object) string.Format("{0}: {1}: More than the specified number of task executions per tick ({2}) have executed, returning early.", (object) this.RoundedTime(), (object) ((object) behaviorTree.behavior).ToString(), (object) this.maxTaskExecutionsPerTick));
            ...
            if (task.CanRunParallelChildren())
            {
              ...
              task.OnChildStarted();
            status = taskStatus = this.RunTask(behaviorTree, intList[num3], stackIndex, status);
          }
        }
        stackIndex = num1;
      }
    ...
    private void PushTask(BehaviorManager.BehaviorTree behaviorTree, int taskIndex, int stackIndex)
    {
      ...
      if (behaviorTree.behavior.LogTaskChanges)
        MonoBehaviour.print((object) string.Format("{0}: {1}: Push task {2} ({3}, index {4}) at stack index {5}", (object) this.RoundedTime(), (object) ((object) behaviorTree.behavior).ToString(), (object) task.FriendlyName, (object) task.GetType(), (object) taskIndex, (object) stackIndex));
      task.OnStart();
      ...
      bool notifyOnEmptyStack)
    {
      ...
      if (behaviorTree.behavior.LogTaskChanges)
        MonoBehaviour.print((object) string.Format("{0}: {1}: Pop task {2} ({3}, index {4}) at stack index {5} with status {6}", (object) this.RoundedTime(), (object) ((object) behaviorTree.behavior).ToString(), (object) task1.FriendlyName, (object) task1.GetType(), (object) taskIndex, (object) stackIndex, (object) status));
      if (index1 != -1)
      {
          ...
          else if (composite.AbortType == AbortType.LowerPriority || composite.AbortType == AbortType.Both)
          {
            ...
            if (num1 != -1)
            {
              if (!(behaviorTree.taskList[num1] as ParentTask).CanRunParallelChildren())
              {
                ...
                {
                  int num2 = behaviorTree.childConditionalIndex[taskIndex][index4];
                  BehaviorManager.BehaviorTree.ConditionalReevaluate conditionalReevaluate3;
                  if (behaviorTree.conditionalReevaluateMap.TryGetValue(num2, out conditionalReevaluate3))
                  {
                    if (!(behaviorTree.taskList[num1] as ParentTask).CanRunParallelChildren())
                    {
                    ...
                    else
                    {
                      ...
                      {
                        BehaviorManager.BehaviorTree.ConditionalReevaluate conditionalReevaluate4 = behaviorTree.conditionalReevaluate[index5];
                        if (this.FindLCA(behaviorTree, num1, conditionalReevaluate4.index) == num1)
                        {
                          behaviorTree.taskList[behaviorTree.conditionalReevaluate[index5].index].NodeData.IsReevaluating = false;
                          ObjectPool.Return<BehaviorManager.BehaviorTree.ConditionalReevaluate>(behaviorTree.conditionalReevaluate[index5]);
                          behaviorTree.conditionalReevaluateMap.Remove(behaviorTree.conditionalReevaluate[index5].index);
    ...
    {
      if (behaviorTree.behavior.LogTaskChanges)
        UnityEngine.Debug.Log((object) string.Format("{0}: Restarting {1}", (object) this.RoundedTime(), (object) ((object) behaviorTree.behavior).ToString()));
      this.RemoveChildConditionalReevaluate(behaviorTree, -1);
        ...
        if (behaviorTree.activeStack[index1].Count > 0)
        {
          for (int index2 = behaviorTree.activeStack[index1].Peek(); index2 != -1; index2 = behaviorTree.parentIndex[index2])
          {
            if (index2 == num)
            {
              ...
              if (behavior.LogTaskChanges)
                UnityEngine.Debug.Log((object) string.Format("{0}: {1}: Interrupt task {2} ({3}) with index {4} at stack index {5}", (object) this.RoundedTime(), (object) ((object) behaviorTree.behavior).ToString(), (object) task.FriendlyName, (object) task.GetType().ToString(), (object) num, (object) index1));
    ...
    public void BehaviorOnCollisionEnter(Collision collision, Behavior behavior)
    {
      ...
      for (int index3 = 0; index3 < behaviorTree.conditionalReevaluate.Count; ++index3)
      {
        ...
        if (!behaviorTree.taskList[index4].Disabled && behaviorTree.conditionalReevaluate[index3].compositeIndex != -1)
      ...
      BehaviorManager.ThirdPartyObjectType objectType)
    {
      if (this.objectTaskMap.ContainsKey(objectKey))
      {
        ...
        UnityEngine.Debug.LogError((object) string.Format("Only one behavior can be mapped to the same instance of the {0}.", (object) str));
        return false;
      }
