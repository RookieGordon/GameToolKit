...
namespace BehaviorDesigner.Runtime.Tasks
{
    ...
    {
        [Tooltip("Text to output to the log")] public SharedString text;
        ...
        public override TaskStatus OnUpdate()
        {
            ...
            if (logError.Value)
            {
                Debug.LogError(logTime.Value ? $"{DateTime.UtcNow.ToString()}: {text}" : text);
            }
            else
            {
                Debug.Log(logTime.Value ? $"{DateTime.UtcNow.ToString()}: {text}" : text);
            }
