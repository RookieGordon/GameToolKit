...
namespace BehaviorDesigner.Runtime
{
    public partial class BehaviorManager
    {
            ...
            get
            {
                if (BehaviorManager.dialogueSystemStopMethod == (MethodInfo)null)
                    BehaviorManager.dialogueSystemStopMethod = TaskUtility
                        .GetTypeWithinAssembly(
                            "BehaviorDesigner.Runtime.BehaviorManager_DialogueSystem"
                        )
            ...
            get
            {
                if (BehaviorManager.uSequencerStopMethod == (MethodInfo)null)
                    BehaviorManager.uSequencerStopMethod = TaskUtility
                        .GetTypeWithinAssembly(
                            "BehaviorDesigner.Runtime.BehaviorManager_uSequencer"
                        )
        ...
        private async Task UpdateIntervalChanged()
        {
            // 等待0.017秒
            await Task.Delay(this.updateIntervalSeconds);
...
#if !UNITY_EDITOR
        public async Task EnableBehavior(Behavior behavior)
        {
            ...
            else if (behavior.AsynchronousLoad)
            {
                BehaviorManager.BehaviorThreadLoader behaviorThreadLoader =
                    new BehaviorManager.BehaviorThreadLoader(
                        behavior,
                        new Func<Behavior, BehaviorManager.BehaviorTree>(this.LoadBehavior)
                    );
        ...
        private BehaviorTree LoadBehavior(Behavior behavior)
        {
            ...
            var gameObjectName = behavior.GetOwnerName();
            if (rootTask == null)
            {
            ...
            if (taskList < 0)
            {
                switch (taskList + 6)
                {
                    ...
                    case 2:
                        behaviorTree.errorState =
                            $"The behavior \"{behavior.GetBehaviorSource().behaviorName}\" on GameObject \"{gameObjectName}\" contains multiple external behavior trees at the root task or as a child of a parent task which cannot contain so many children (such as a decorator task). This behavior will be disabled.";
                        break;
                    case 3:
                        behaviorTree.errorState =
        ...
        private void LoadBehaviorComplete(Behavior behavior, BehaviorTree behaviorTree)
        {
            ...
            else
            {
                ...
                if (behaviorTree.behavior.LogTaskChanges)
                {
                    for (int index = 0; index < behaviorTree.taskList.Count; ++index)
                        Debug.Log(
                                $"{(object)this.RoundedTime()}: Task {(object)behaviorTree.taskList[index].FriendlyName} ({(object)behaviorTree.taskList[index].GetType()}, index {(object)index}) {(object)behaviorTree.taskList[index].GetHashCode()}"
                            );
                }
                for (int index = 0; index < behaviorTree.taskList.Count; ++index)
        ...
        )
        {
            ...
            SetTaskGameObject(task, behavior);
        ...
        )
        {
            ...
            for (int index1 = 0; index1 < serializableFields.Length; ++index1)
            {
                ...
                {
                    if (
                        typeof(SharedVariable).IsAssignableFrom(
                            serializableFields[index1].FieldType
                        )
                    )
                    {
                        SharedVariable sharedVariable = this.OverrideSharedVariable(
                            behaviorTree,
                            data,
                            serializableFields[index1].FieldType,
                    ...
                    else if (typeof(IList).IsAssignableFrom(serializableFields[index1].FieldType))
                    {
                        ...
                        if (
                            (
                                typeof(SharedVariable).IsAssignableFrom(
                                    fieldType = serializableFields[index1]
                                        .FieldType
                                        .GetElementType()
                                )
                        ...
                        if (typeof(IList).IsAssignableFrom(serializableFields[index1].FieldType))
                        {
                            if (
                                typeof(Task).IsAssignableFrom(
                                    serializableFields[index1].FieldType.GetElementType()
                                ) && obj1 is IList list1
                            )
                            {
        ...
        public void DisableBehavior(Behavior behavior, bool paused, TaskStatus executionStatus)
        {
            if (!this.IsBehaviorEnabled(behavior))
            {
                ...
                else
                {
                    ...
                    {
                        if (
                            (UnityEngine.Object)this.activeThreads[index].Behavior
                            == (UnityEngine.Object)behavior
                        )
                        {
                    ...
                    .Log(
                        (object)
                            string.Format(
                                ...
                                !paused ? (object)"Disabling" : (object)"Pausing",
                                (object)((object)behavior).ToString()
                            )
                    );
            if (paused)
            {
                BehaviorManager.BehaviorTree behaviorTree;
                if (
                    !this.behaviorTreeMap.TryGetValue(behavior, out behaviorTree)
                    || this.pausedBehaviorTrees.ContainsKey(behavior)
                )
                    return;
        ...
        private void Tick(BehaviorManager.BehaviorTree behaviorTree)
        {
            ...
            for (int index = 0; index < count1; ++index)
            {
                ...
                int num1;
                if (
                    index < behaviorTree.interruptionIndex.Count
                    && (num1 = behaviorTree.interruptionIndex[index]) != -1
                )
                {
                    ...
                    if (taskStatus != behaviorTree.conditionalReevaluate[index1].taskStatus)
                    {
                        if (behaviorTree.behavior.LogTaskChanges)
                        {
                            ...
                            MonoBehaviour.print(
                                (object)
                                    string.Format(
                                        "{0}: {1}: Conditional abort with task {2} ({3}, index {4}) because of conditional task {5} ({6}, index {7}) with status {8}",
                                        (object)this.RoundedTime(),
                                        (object)((object)behaviorTree.behavior).ToString(),
                                        (object)behaviorTree.taskList[index3].FriendlyName,
                                        (object)behaviorTree.taskList[index3].GetType(),
                                        (object)index3,
                                        (object)behaviorTree.taskList[index2].FriendlyName,
                                        (object)behaviorTree.taskList[index2].GetType(),
                                        (object)index2,
                                        (object)taskStatus
                                    )
                            );
        ...
        private void ReevaluateParentTasks(BehaviorManager.BehaviorTree behaviorTree)
        {
            for (int index = behaviorTree.parentReevaluate.Count - 1; index > -1; --index)
            {
                ...
                else if (behaviorTree.taskList[num] is Composite)
                {
                    ...
                    {
                        int stackIndex = 0;
                        TaskStatus status = this.RunParentTask(
                            behaviorTree,
                            num,
                            ref stackIndex,
                            TaskStatus.Inactive
        ...
        )
        {
            ...
            if (task1.Disabled)
            {
                if (behaviorTree.behavior.LogTaskChanges)
                    MonoBehaviour.print(
                        (object)
                            string.Format(
                                "{0}: {1}: Skip task {2} ({3}, index {4}) at stack index {5} (task disabled)",
                                (object)this.RoundedTime(),
                                (object)((object)behaviorTree.behavior).ToString(),
                                (object)behaviorTree.taskList[taskIndex].FriendlyName,
                                (object)behaviorTree.taskList[taskIndex].GetType(),
                                (object)taskIndex,
                                (object)stackIndex
                            )
                    );
                if (behaviorTree.parentIndex[taskIndex] != -1)
                {
                    ...
                    {
                        task2.OnChildExecuted(
                            behaviorTree.relativeChildIndex[taskIndex],
                            TaskStatus.Inactive
                        );
            ...
            TaskStatus status2 = !(task1 is ParentTask)
                ? task1.OnUpdate()
                : (task1 as ParentTask).OverrideStatus(
                    this.RunParentTask(behaviorTree, taskIndex, ref stackIndex, status1)
                );
                ...
                )
                {
                    ...
                    else
                    {
                            ...
                            if (
                                ...
                                UnityEngine
                                    ...
                                    .LogWarning(
                                        (object)
                                            string.Format(
                                                "{0}: {1}: More than the specified number of task executions per tick ({2}) have executed, returning early.",
                                                (object)this.RoundedTime(),
                                                (object)((object)behaviorTree.behavior).ToString(),
                                                (object)this.maxTaskExecutionsPerTick
                                            )
                                    );
                            status = TaskStatus.Running;
                            break;
                        ...
                        if (task.CanRunParallelChildren())
                        {
                            ...
                            task.OnChildStarted();
                        status = taskStatus = this.RunTask(
                            behaviorTree,
                            intList[num3],
                            stackIndex,
                            status
        ...
        )
        {
            ...
            if (behaviorTree.behavior.LogTaskChanges)
                MonoBehaviour.print(
                    (object)
                        string.Format(
                            "{0}: {1}: Push task {2} ({3}, index {4}) at stack index {5}",
                            (object)this.RoundedTime(),
                            (object)((object)behaviorTree.behavior).ToString(),
                            (object)task.FriendlyName,
                            (object)task.GetType(),
                            (object)taskIndex,
                            (object)stackIndex
                        )
        ...
        )
        {
            ...
            if (behaviorTree.behavior.LogTaskChanges)
                MonoBehaviour.print(
                    (object)
                        string.Format(
                            "{0}: {1}: Pop task {2} ({3}, index {4}) at stack index {5} with status {6}",
                            (object)this.RoundedTime(),
                            (object)((object)behaviorTree.behavior).ToString(),
                            (object)task1.FriendlyName,
                            (object)task1.GetType(),
                            (object)taskIndex,
                            (object)stackIndex,
                            (object)status
                        )
                    ...
                    if (index2 != -1)
                    {
                        ...
                        if (task2.AbortType != AbortType.None)
                        {
                            ...
                            else
                            {
                                ...
                                conditionalReevaluate2.Initialize(
                                    taskIndex,
                                    status,
                                    stackIndex,
                                    task2.AbortType == AbortType.LowerPriority ? -1 : index2
        ...
        )
        {
            for (int index1 = behaviorTree.conditionalReevaluate.Count - 1; index1 > -1; --index1)
            {
                ...
                {
                    ObjectPool.Return<BehaviorManager.BehaviorTree.ConditionalReevaluate>(
                        behaviorTree.conditionalReevaluate[index1]
                    );
                    int index2 = behaviorTree.conditionalReevaluate[index1].index;
                            ...
                            if (behavior.LogTaskChanges)
                                UnityEngine
                                    ...
                                    .Log(
                                        (object)
                                            string.Format(
                                                "{0}: {1}: Interrupt task {2} ({3}) with index {4} at stack index {5}",
                                                (object)this.RoundedTime(),
                                                (object)((object)behaviorTree.behavior).ToString(),
                                                (object)task.FriendlyName,
                                                (object)task.GetType().ToString(),
                                                (object)num,
                                                (object)index1
                                            )
                                    );
            ...
            if (this.objectTaskMap.ContainsKey(objectKey))
            {
                ...
                UnityEngine
                    ...
                    .LogError(
                        (object)
                            string.Format(
                                "Only one behavior can be mapped to the same instance of the {0}.",
                                (object)str
                            )
                    );
                return false;
        ...
        private Decimal RoundedTime() =>
            Math.Round((Decimal)Time.time, 5, MidpointRounding.AwayFromZero);
