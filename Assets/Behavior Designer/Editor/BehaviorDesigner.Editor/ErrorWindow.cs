                    ...
                    if (mErrorDetail != null && (mErrorDetail.Type == BehaviorDesigner.Editor.ErrorDetails.ErrorType.InvalidVariableReference || !((UnityEngine.Object)mErrorDetail.NodeDesigner == (UnityEngine.Object)null) && mErrorDetail.NodeDesigner.Task != null))
                    {
                        ...
                        switch (mErrorDetail.Type)
                        {
                            case BehaviorDesigner.Editor.ErrorDetails.ErrorType.RequiredField:
                                str = string.Format("The task {0} ({1}, index {2}) requires a value for the field {3}.",
                                    (object)mErrorDetail.TaskFriendlyName,
                                    (object)mErrorDetail.TaskType,
                                    (object)mErrorDetail.NodeDesigner.Task.ID,
                                    (object)BehaviorDesignerUtility.SplitCamelCase(mErrorDetail.FieldName));
                                break;
                            case BehaviorDesigner.Editor.ErrorDetails.ErrorType.SharedVariable:
                                str = string.Format("The task {0} ({1}, index {2}) has a Shared Variable field ({3}) that is marked as shared but is not referencing a Shared Variable.",
                                    (object)mErrorDetail.TaskFriendlyName,
                                    (object)mErrorDetail.TaskType,
                                    (object)mErrorDetail.NodeDesigner.Task.ID,
                                    (object)BehaviorDesignerUtility.SplitCamelCase(mErrorDetail.FieldName));
                            ...
                            case BehaviorDesigner.Editor.ErrorDetails.ErrorType.NonUniqueDynamicVariable:
                                str = string.Format("The task {0} ({1}, index {2}) has a dynamic Shared Variable ({3}) but the name matches an existing Shared Varaible.",
                                    (object)mErrorDetail.TaskFriendlyName,
                                    (object)mErrorDetail.TaskType,
                                    (object)mErrorDetail.NodeDesigner.Task.ID,
                                    (object)BehaviorDesignerUtility.SplitCamelCase(mErrorDetail.FieldName));
                            ...
                            case BehaviorDesigner.Editor.ErrorDetails.ErrorType.InvalidTaskReference:
                                str = string.Format("The task {0} ({1}, index {2}) has a field ({3}) which is referencing an object within the scene. Behavior tree variables at the project level cannot reference objects within a scene.",
                                    (object)mErrorDetail.TaskFriendlyName,
                                    (object)mErrorDetail.TaskType,
                                    (object)mErrorDetail.NodeDesigner.Task.ID,
                                    (object)BehaviorDesignerUtility.SplitCamelCase(mErrorDetail.FieldName));
                        ...
                        EditorGUILayout.LabelField(str,
                            index % 2 != 0 ? BehaviorDesignerUtility.ErrorListDarkBackground : BehaviorDesignerUtility.ErrorListLightBackground,
