...
namespace BehaviorDesigner.Editor
{
    ...
    public class TaskInspector : ScriptableObject
    {
            ...
            bool enabled)
        {
            ...
            GUILayout.BeginHorizontal(Array.Empty<GUILayoutOption>());
            EditorGUILayout.LabelField("Instant",
                new GUILayoutOption[1]
                {
                    GUILayout.Width(90f)
                });
                ...
                for (int index1 = baseClasses.Count - 1; index1 > -1; --index1)
                {
                    ...
                    for (int index2 = 0; index2 < fields.Length; ++index2)
                    {
                        if (!BehaviorDesignerUtility.HasAttribute(fields[index2], typeof(NonSerializedAttribute)) && !BehaviorDesignerUtility.HasAttribute(fields[index2], typeof(HideInInspector)) &&
                            ...
                            (!isReflectionTask || !fields[index2].FieldType.Equals(typeof(SharedVariable)) && !fields[index2].FieldType.IsSubclassOf(typeof(SharedVariable)) || this.CanDrawReflectedField(obj, fields[index2])))
                        {
                            ...
                            GUIContent guiContent = (customAttributes3 = fields[index2].GetCustomAttributes(typeof(TooltipAttribute), false) as TooltipAttribute[]).Length <= 0
                                ? new GUIContent(BehaviorDesignerUtility.SplitCamelCase(s))
                                : new GUIContent(BehaviorDesignerUtility.SplitCamelCase(s), customAttributes3[0].Tooltip);
                            ...
                            if (typeof(Task).IsAssignableFrom(fieldType) || typeof(IList).IsAssignableFrom(fieldType) && (typeof(Task).IsAssignableFrom(fieldType.GetElementType()) || fieldType.IsGenericType && typeof(Task).IsAssignableFrom(fieldType.GetGenericArguments()[0])))
                            {
                                ...
                                if (BehaviorDesignerWindow.instance.ContainsError(task, fields[index2].Name))
                                {
                                    ...
                                    GUILayout.Box((Texture)BehaviorDesignerUtility.ErrorIconTexture,
                                        BehaviorDesignerUtility.PlainTextureGUIStyle,
                                        new GUILayoutOption[1]
                                        {
                                            GUILayout.Width(20f)
                                        });
        ...
        private bool DrawWatchedButton(Task task, FieldInfo field)
        {
            ...
            if (!GUILayout.Button((Texture)(!flag ? BehaviorDesignerUtility.VariableWatchButtonTexture : BehaviorDesignerUtility.VariableWatchButtonSelectedTexture),
            ...
            bool enabled)
        {
            if (BehaviorDesignerUtility.HasAttribute(field, typeof(InspectTaskAttribute)))
            {
                ...
                if (GUILayout.Button(task == null ? "Select" : BehaviorDesignerUtility.SplitCamelCase(((object)task).GetType().Name.ToString()),
            ...
            else
            {
                ...
                GUILayout.Label(guiContent,
                    BehaviorDesignerUtility.TaskInspectorGUIStyle,
                    new GUILayoutOption[1]
                    {
                ...
                if (GUILayout.Button(!flag ? "Select" : "Done",
                    ...
                    else
                    {
                        for (int index = 0; index < list.Count; ++index)
                        {
                            if (list[index] is Task)
                            {
                                ...
                                GUILayout.Label((list[index] as Task).NodeData.NodeDesigner.ToString(),
                                    BehaviorDesignerUtility.TaskInspectorGUIStyle,
                                    new GUILayoutOption[1]
                                    {
                                        GUILayout.Width(232f)
                                    });
                                if (GUILayout.Button((Texture)BehaviorDesignerUtility.DeleteButtonTexture,
                                        BehaviorDesignerUtility.PlainButtonGUIStyle,
                                        new GUILayoutOption[1]
                                        {
                                            GUILayout.Width(14f)
                                ...
                                GUILayout.Space(3f);
                                if (GUILayout.Button((Texture)BehaviorDesignerUtility.IdentifyButtonTexture,
                                        BehaviorDesignerUtility.PlainButtonGUIStyle,
                                        new GUILayoutOption[1]
                                        {
                                            GUILayout.Width(14f)
                ...
                else
                {
                    ...
                    GUILayout.Label(task1 == null ? "No Tasks Referenced" : task1.NodeData.NodeDesigner.ToString(),
                        BehaviorDesignerUtility.TaskInspectorGUIStyle,
                        new GUILayoutOption[1]
                        {
                            GUILayout.Width(232f)
                    ...
                    if (index != num)
                    {
                        if (index == 0)
                        {
                            sharedVariable = !field.FieldType.Equals(typeof(SharedVariable))
                                ...
                                : Activator.CreateInstance(FieldInspector.FriendlySharedVariableName(((object)sharedVariable).GetType().GetProperty("Value").PropertyType)) as SharedVariable;
                            sharedVariable.IsShared = true;
                        }
                        else
        ...
        private void InspectedTaskCallback(object obj)
        {
            if (this.mActiveMenuSelectionTask != null)
            {
                ...
                else
                {
                    ...
                    {
                        if (serializableFields[index].FieldType.IsSubclassOf(typeof(SharedVariable)) && !BehaviorDesignerUtility.HasAttribute(serializableFields[index], typeof(HideInInspector)) && !BehaviorDesignerUtility.HasAttribute(serializableFields[index], typeof(NonSerializedAttribute)) &&
                            (!serializableFields[index].IsPrivate && !serializableFields[index].IsFamily || BehaviorDesignerUtility.HasAttribute(serializableFields[index], typeof(SerializeField))))
                        {
            ...
            bool unreferenceAll)
        {
            ...
            if (((object)referenceTask).Equals((object)sourceTask) || sourceFieldInfo == (FieldInfo)null || !typeof(IList).IsAssignableFrom(sourceFieldInfo.FieldType) && !sourceFieldInfo.FieldType.IsAssignableFrom(((object)referenceTask).GetType()) ||
                typeof(IList).IsAssignableFrom(sourceFieldInfo.FieldType) && (sourceFieldInfo.FieldType.IsGenericType && !sourceFieldInfo.FieldType.GetGenericArguments()[0].IsAssignableFrom(((object)referenceTask).GetType()) ||
                                                                              !sourceFieldInfo.FieldType.IsGenericType && !sourceFieldInfo.FieldType.GetElementType().IsAssignableFrom(((object)referenceTask).GetType())))
                return false;
        ...
        public static List<Task> GetReferencedTasks(Task task)
        {
            ...
            for (int index1 = 0; index1 < serializableFields.Length; ++index1)
            {
                ...
                {
                    if (typeof(IList).IsAssignableFrom(serializableFields[index1].FieldType) && (typeof(Task).IsAssignableFrom(serializableFields[index1].FieldType.GetElementType()) ||
                                                                                                 serializableFields[index1].FieldType.IsGenericType && typeof(Task).IsAssignableFrom(serializableFields[index1].FieldType.GetGenericArguments()[0])))
                    {
        ...
        private bool IsFieldReflectionTask(System.Type type) =>
            TaskUtility.CompareType(type, "BehaviorDesigner.Runtime.Tasks.GetFieldValue") || TaskUtility.CompareType(type, "BehaviorDesigner.Runtime.Tasks.SetFieldValue") || TaskUtility.CompareType(type, "BehaviorDesigner.Runtime.Tasks.CompareFieldValue");

        private bool IsPropertyReflectionTask(System.Type type) => TaskUtility.CompareType(type, "BehaviorDesigner.Runtime.Tasks.GetPropertyValue") || TaskUtility.CompareType(type, "BehaviorDesigner.Runtime.Tasks.SetPropertyValue") ||
            ...
            FieldInfo field)
        {
            ...
            else
            {
                ...
                string empty = string.Empty;
                if (GUILayout.Button(sharedVariable3 == null || string.IsNullOrEmpty((string)sharedVariable3.GetValue()) ? "Component Required" : (!string.IsNullOrEmpty((string)sharedVariable4.GetValue()) ? (string)sharedVariable4.GetValue() : "Select"),
                        EditorStyles.toolbarPopup,
                        new GUILayoutOption[1]
                        {
                            GUILayout.Width(92f)
        ...
        private bool CanDrawReflectedField(object task, FieldInfo field)
        {
            ...
            return this.IsFieldReflectionTask(task.GetType())
                ? task.GetType().GetField("fieldName").GetValue(task) is SharedVariable sharedVariable1 && !string.IsNullOrEmpty((string)sharedVariable1.GetValue())
                : task.GetType().GetField("propertyName").GetValue(task) is SharedVariable sharedVariable2 && !string.IsNullOrEmpty((string)sharedVariable2.GetValue());
        }
