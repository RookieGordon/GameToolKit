  ...
  public static class FieldInspector
  {
        ...
        for (int index2 = 0; index2 < fields.Length; ++index2)
        {
          if (!BehaviorDesignerUtility.HasAttribute(fields[index2], typeof (NonSerializedAttribute)) && !BehaviorDesignerUtility.HasAttribute(fields[index2], typeof (HideInInspector)) && (!fields[index2].IsPrivate && !fields[index2].IsFamily || BehaviorDesignerUtility.HasAttribute(fields[index2], typeof (SerializeField))) && (!(obj is ParentTask) || !fields[index2].Name.Equals("children")))
          {
            if (guiContent == null)
            {
              ...
              guiContent = (customAttributes = fields[index2].GetCustomAttributes(typeof (TooltipAttribute), false) as TooltipAttribute[]).Length <= 0 ? new GUIContent(BehaviorDesignerUtility.SplitCamelCase(name)) : new GUIContent(BehaviorDesignerUtility.SplitCamelCase(name), customAttributes[0].Tooltip);
            }
      ...
      SharedVariable sharedVariable)
    {
      ...
      else if (sharedVariable == null || sharedVariable.IsShared)
      {
        ...
        int variablesOfType = FieldInspector.GetVariablesOfType(sharedVariable == null ? (System.Type) null : ((object) sharedVariable).GetType().GetProperty("Value").PropertyType, sharedVariable != null && sharedVariable.IsGlobal, sharedVariable == null ? string.Empty : sharedVariable.Name, FieldInspector.behaviorSource, out names, ref globalStartIndex, fieldType.Equals(typeof (SharedVariable)), addDynamic);
      ...
      SharedVariable sharedVariable)
    {
      ...
      if (GUILayout.Button((Texture) (!sharedVariable.IsShared ? BehaviorDesignerUtility.VariableButtonTexture : BehaviorDesignerUtility.VariableButtonSelectedTexture), BehaviorDesignerUtility.PlainButtonGUIStyle, new GUILayoutOption[1]
      {
        GUILayout.Width(15f)
      }))
      {
