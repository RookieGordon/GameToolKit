    ...
    public void Init()
    {
      ...
      foreach (Assembly assembly in AppDomain.CurrentDomain.GetAssemblies())
      {
        try
        {
          ...
          {
            if (typeof (Task).IsAssignableFrom(types[index]) && !types[index].Equals(typeof (BehaviorReference)) && !types[index].IsAbstract && (types[index].IsSubclassOf(typeof (Runtime.Tasks.Action)) || types[index].IsSubclassOf(typeof (Composite)) || types[index].IsSubclassOf(typeof (Conditional)) || types[index].IsSubclassOf(typeof (Decorator))))
      ...
      if (this.mCategoryList[typeIndex].Tasks != null)
      {
        for (int index = 0; index < this.mCategoryList[typeIndex].Tasks.Count; ++index)
        {
          if (!(this.mCategoryList[typeIndex].Tasks[index].Type == ignoredTaskType))
          {
            ...
            {
              if (includeFullPath)
                genericMenu.AddItem(new GUIContent(string.Format("{0}/{1}", (object) this.mCategoryList[typeIndex].Fullpath, (object) this.mCategoryList[typeIndex].Tasks[index].Name.ToString())), this.mCategoryList[typeIndex].Tasks[index].Type.Equals(selectedTaskType), menuFunction, (object) this.mCategoryList[typeIndex].Tasks[index].Type);
              else
              ...
              genericMenu.AddItem(new GUIContent(string.Format("{0}/{1}/{2}", (object) parentName, (object) this.mCategoryList[typeIndex].Fullpath, (object) this.mCategoryList[typeIndex].Tasks[index].Name.ToString())), this.mCategoryList[typeIndex].Tasks[index].Type.Equals(selectedTaskType), menuFunction, (object) this.mCategoryList[typeIndex].Tasks[index].Type);
            else
              genericMenu.AddItem(new GUIContent(string.Format("{0}/{1}", (object) parentName, (object) this.mCategoryList[typeIndex].Tasks[index].Name.ToString())), this.mCategoryList[typeIndex].Tasks[index].Type.Equals(selectedTaskType), menuFunction, (object) this.mCategoryList[typeIndex].Tasks[index].Type);
          }
        }
      }
      ...
      if (category.Tasks != null)
      {
        for (int index = 0; index < category.Tasks.Count; ++index)
        {
          if (category.Tasks[index].Visible)
          {
            ...
            if (GUILayout.Button(str3, EditorStyles.miniButton, new GUILayoutOption[1]
            {
