...
namespace Bonsai.Designer
{
    ...
    public class BonsaiCanvas
    {
        ...
        {
            Tree = tree;
            var nodeMap = ReconstructEditorNodes(tree.Nodes.Concat(tree.Proxy.UnusedNodes));
        ...
        public BonsaiNode CreateNode(Type behaviourType) 
        {
            ...
            var proxy = ScriptableObject.CreateInstance(proxyType);
            var nodeProxy = proxy as BehaviourNodeProxy;
            return CreateNode(nodeProxy.Node);
        }
        ...
        private BonsaiNode ReconstructEditorNode(BehaviourNode behaviour)
        {
            ...
            node.Behaviour = behaviour;
            node.Position = behaviour.Proxy.bonsaiNodePosition;
            return node;
        }
