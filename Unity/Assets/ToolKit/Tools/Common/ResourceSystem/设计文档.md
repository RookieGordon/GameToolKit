...
错误不再是裸 `Exception`，而是「预定义错误码 + 人类可读信息」。`ELoadError` 枚举给出分类（`NotFound`/`NetworkError`/`IOError`/`InvalidAddress`/`NoLoader`/`NotInstantiable`/`InstancerMissing`/`Disposed`/`Cancelled` 等）；`LoadError` 结构封装 `Code + Message + Inner`（原始异常留作诊断，可空）。

错误按性质走两条路。**运行时加载失败**（文件不存在、下载失败、读盘错误、被取消）是「数据」，不抛异常：挂在 `IAssetHandle.Error` 上；并且 `LoadRefAsync` 失败时返回一张**失败凭证**——`ref.IsValid == false`、`ref.Error` 可读、`Get` 返回 null、`Dispose` 是安全空操作且不进登记表（不会泄漏）。业务 `await` 完直接 `if (!ref.IsValid) Log(ref.Error)` 即可，无需 try/catch。各 loader 在失败处设具体码：本地文件不存在→`NotFound`、远端下载失败→`NetworkError`、读盘异常→`IOError`、Bundle 地址格式错→`InvalidAddress`，原始异常塞进 `Inner`。

**配置/误用错误**（找不到加载器、可应用资源被拿去池化、未注册实例器、管理器已释放）是「程序员用错了」，仍 fail-fast 抛出，但抛的是带 `Code` 的 typed `ResourceException`（取代裸 `InvalidOperationException`/`ObjectDisposedException`），异常本身就是分类、可读的。唯一例外是凭证重复释放：`Dispose` 必须安全，故只记 `Log.Error` 不抛。错误流向一句话：能加载到就是成功凭证、加载不到就是带 `Error` 的失败凭证、压根用错了就抛 `ResourceException`。
...
{
    if (!refObj.IsValid)                 // 失败也返回凭证, 检查即可, 无需 try/catch
    {
        Debug.LogError(refObj.Error);    // 输出形如 [NotFound] ...
        return;
    }
    var prefab = refObj.Get<GameObject>();
    // ... 使用 ...
} // 离开作用域自动 Dispose -> 句柄引用 -1 -> (延迟)卸载; 失败凭证 Dispose 为空操作
```
