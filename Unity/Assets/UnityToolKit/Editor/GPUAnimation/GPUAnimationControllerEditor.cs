...
namespace UnityToolKit.Editor.GPUAnimation
{
    ...
    public class GPUAnimationControllerEditor : CustomInspector
    {
        ...
        public override bool HasPreviewGUI()
        {
            ...
            if (controller.GPUAnimData == null)
            {
                return false;
            }
        ...
        protected override void OnEnable()
        {
            ...
            _previewTarget.Init();
            if (_previewTarget.AnimationCount > 0)
        ...
        {
            // 从所有数据集构建统一动画列表
            int totalAnims = _previewTarget.AnimationCount;
            string[] anims = new string[totalAnims];
            for (int i = 0; i < totalAnims; i++)
            {
                string name = _previewTarget.GetAnimationName(i);
                anims[i] = name.Substring(name.LastIndexOf("_") + 1);
            }

            GUILayout.BeginHorizontal();
            GUILayout.Label("Play:");
            int newIndex = GUILayout.SelectionGrid(_previewAnimIndex, anims,
                totalAnims > 5 ? 5 : totalAnims);
            if (newIndex != _previewAnimIndex)
            {
                _previewAnimIndex = newIndex;
        ...
        void Update()
        {
            ...
            // 只驱动预览克隆体，不要 Tick 场景中的真实对象（它有自己的 Update 驱动）
